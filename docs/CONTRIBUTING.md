...
| Tag 形如        | 含义           | 触发动作                                              |
| --------------- | -------------- | ----------------------------------------------------- |
| `v1.2.3`        | 正式版         | 构建 exe 发布 GitHub Release（正式 Release）          |
| `beta1` `rc2` `alpha1` | 预发布 | 构建 exe 发布 GitHub Release（标记为 pre-release）    |

> 本工具是**面向终端用户的 GUI 程序**；分发靠 GitHub Releases 的
> 单文件 `AiEnvClone.exe` + 自动附带的源码包。
...
- **`build-release.yml`**（`windows-latest` 构建 `AiEnvClone.exe`）：
  - 下载产物 → 自动创建 GitHub Release → 把 `AiEnvClone.exe` 作为资产附件上传；
...
> single-file `AiEnvClone.exe` on GitHub Releases plus the auto-attached source archives.
...
- **`build-release.yml`** (`windows-latest` builds `AiEnvClone.exe`):
  - Downloads the artifact → creates the GitHub Release → uploads `AiEnvClone.exe` as a release asset;
