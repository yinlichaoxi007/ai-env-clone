...
- **高分屏适配**：窗口声明 DPI 感知（Per-Monitor v2），缩放系数超过 100% 时不会被系统虚化放大；备份内容区高度统一为固定值（与工具/项数无关，三工具切换时内容区与主窗口高度完全一致，占屏约 3/4），内容过多时由滚动条承载，不会显示不全。
...
- **Hi-DPI support**: the window declares DPI awareness (Per-Monitor v2), so it is not blurry-scaled by the OS when the scaling factor exceeds 100%. The backup content area uses a fixed uniform height (independent of tool/item count, so switching tools keeps the content area and main window height identical across all three tools, occupying about 3/4 of the screen), with a scrollbar for overflow, so the window never gets clipped.
